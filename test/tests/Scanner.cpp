...
template <hat::detail::scan_mode Mode, size_t SignatureSize,
...
template <typename T> class FindPatternTest;

template <typename T>
concept FindPatternTestCallback =
    std::invocable<T, hat::const_scan_result, /* actual */
                   const std::byte *          /* expected */
                   >;

template <hat::detail::scan_mode Mode, size_t SignatureSize,
          size_t MaxBufferSize>
class FindPatternTest<FindPatternParameters<Mode, SignatureSize, MaxBufferSize>>
    : public ::testing::Test {
protected:
  void run_cases(const hat::scan_alignment alignment,
    ...
    const auto contextA = hat::detail::scan_context::create<Mode>(
        sig, alignment, hat::scan_hint::none);
    const auto contextB = hat::detail::scan_context::create<Mode>(
        sigWildcard, alignment, hat::scan_hint::none);
    ...
    for (size_t size = SignatureSize; size != MaxBufferSize; size++) {
      for (size_t offset{}; offset != size - SignatureSize + 1; offset++) {
        ...
        std::ranges::copy(
            sig | std::views::transform(&hat::signature_element::value),
            begin + offset);
...
public:
  template <typename T> static std::string GetName(int) {
    return std::format("{}/{}/{}", getModeName<T::mode>(), T::signature_size,
                       T::max_buffer_size);
  }
...
private:
  ...
  static consteval std::string_view getModeName() {
    if constexpr (Mode == hat::detail::scan_mode::Single)
      return "Single";
    else if constexpr (Mode == hat::detail::scan_mode::SSE)
      return "SSE";
    else if constexpr (Mode == hat::detail::scan_mode::AVX2)
      return "AVX2";
    else if constexpr (Mode == hat::detail::scan_mode::AVX512)
      return "AVX512";
    else if constexpr (Mode == hat::detail::scan_mode::NEON)
      return "NEON";
    else
      static_assert(sizeof(Mode) == 0);
  }
};

TYPED_TEST_SUITE(FindPatternTest, FindPatternTestTypes,
                 FindPatternTestNameGenerator);
